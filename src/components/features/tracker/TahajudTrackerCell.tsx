...
export function TahajudTrackerCell({ day }: Props) {
  const dispatch = useAppDispatch();
  const userId = useAppSelector((s) => s.auth.session?.user?.id ?? s.auth.user?.id ?? "");
  const value = day.tahajud ?? 0;
  const [open, setOpen] = useState(false);

  const handleSelect = (n: number) => {
    const previous = day.tahajud;
    const next = n === 0 ? null : n;
    dispatch(updateDayField({ dayNumber: day.day_number, field: "tahajud", value: next }));
    dispatch(
      updateTrackerField({
        userId,
        year: day.year,
